...
data_long <- melt(data_cif, id.vars = "time")
techs$timepoint <- as.factor(techs$timepoint)
techs$id <- as.factor(techs$id)
ggplot(techs, aes(x = timepoint, y = bess_sum, group = id, color = id)) +
  geom_line() +
  labs(title = "Bess", x = "Timepoint", y = "Total # of Errors")

ggplot(data_hr, aes(x = time, y = HR_perc, group = ID, color = ID)) +
  geom_line() +
  labs(title = "HR Max across intervention", x = "30 Minute session", y = "% of HR max")
