...
table1(~ factor(sex_factor) + age + factor(annual_income_factor) + days_since_injury + ethnicity_factor + factor(race_factor) + years_of_ed | factor(randomization_group_factor), data=techs)
...
```{r pre/post data}
long_df_vf_a <- techs %>%
  select(name, randomization_group_factor, verbal_fluency_a, verbal_fluency_a_v2) %>%
...
ggplot(long_df_vf_a, aes(x = timepoint, y = value, group = name, color = as.factor(randomization_group_factor))) +
  geom_line(linetype = "dashed") +  # Set individual lines as dashed
  theme_bw() +
  theme_classic(base_size = 15) +
 geom_smooth(aes(group = randomization_group_factor), method = "lm", se = FALSE) +  # Adding best fit line by group
  scale_color_manual(values = c("black", "red")) +  # Setting color scale
  labs(title = "Verbal Fluency A", x = "Timepoint", y = "# of words")
long_df_vf_f <- techs %>%
  select(name, randomization_group_factor, verbal_fluency_f, verbal_fluency_f_v2) %>%
...
ggplot(long_df_vf_f, aes(x = timepoint, y = value, group = name, color = as.factor(randomization_group_factor))) +
  geom_line() +
  theme_bw() +
  theme_classic(base_size = 15) +
 geom_smooth(aes(group = randomization_group_factor), method = "lm", se = FALSE) +  # Adding best fit line by group
...
long_df_vf_cat <- techs %>%
  select(name, randomization_group_factor, verbal_fluency_animals, verbal_fluency_animals_v2) %>%
...
ggplot(long_df_vf_cat, aes(x = timepoint, y = value, group = name, color = as.factor(randomization_group_factor))) +
  geom_line(linetype = "dashed") +  # Set individual lines as dashed
  theme_bw() +
  theme_classic(base_size = 15) +
  geom_smooth(aes(group = randomization_group_factor), method = "lm", se = FALSE, size = 1.5) +
  scale_color_manual(values = c("black", "red")) +  # Setting color scale
  labs(title = "Verbal Fluency Categories", x = "Timepoint", y = "# of words")
long_df_vf_all <- techs %>%
  select(name, randomization_group_factor, verbal_fluency_all, verbal_fluency_all_v2) %>%
...
ggplot(long_df_vf_all, aes(x = timepoint, y = value, group = name, color = as.factor(randomization_group_factor))) +
  geom_line(linetype = "dashed") +  # Set individual lines as dashed
   theme_bw() +
  theme_classic(base_size = 15) +
  geom_smooth(aes(group = randomization_group_factor), method = "lm", se = FALSE, size = 1.5) +
...
```{r pre/post data}
long_df_vo2 <- techs %>%
  select(name, randomization_group_factor, relative_vo2max_agecorrected_baseline, relative_vo2max_agecorrected_end) %>%
...
ggplot(long_df_vo2, aes(x = timepoint, y = value, group = name, color = as.factor(randomization_group_factor))) +
  geom_line(linetype = "dashed") +  # Set individual lines as dashed
   theme_bw() +
  theme_classic(base_size = 15) +
  geom_smooth(aes(group = randomization_group_factor), method = "lm", se = FALSE) +  # Adding best fit line by group
...
long_df_bess <- techs %>%
  select(name, randomization_group_factor, BESS, BESS_v2) %>%
  gather(key = "timepoint", value = "value", BESS, BESS_v2)
long_df_bess <- long_df_bess %>%
  mutate(timepoint = ifelse(timepoint == "BESS", "1", "2"))
ggplot(long_df_bess, aes(x = timepoint, y = value, group = name, color = as.factor(randomization_group_factor))) +
  geom_line(linetype = "dashed") +  # Set individual lines as dashed
   theme_bw() +
  theme_classic(base_size = 15) +
  geom_smooth(aes(group = randomization_group_factor), method = "lm", se = FALSE) +  # Adding best fit line by group
...
```{r pre/post data}
long_df_tmt_a <- techs %>%
  select(name, randomization_group_factor, tm_time_a, tm_time_a_v2) %>%
...
ggplot(long_df_tmt_a, aes(x = timepoint, y = value, group = name, color = as.factor(randomization_group_factor))) +
  geom_line(linetype = "dashed") +  # Set individual lines as dashed
   theme_bw() +
  theme_classic(base_size = 15) +
  geom_smooth(aes(group = randomization_group_factor), method = "lm", se = FALSE) +  # Adding best fit line by group
...
long_df_tmt_b <- techs %>%
  select(name, randomization_group_factor, tm_time_b, tm_time_b_v2) %>%
...
ggplot(long_df_tmt_b, aes(x = timepoint, y = value, group = name, color = as.factor(randomization_group_factor))) +
  geom_line(linetype = "dashed") +  # Set individual lines as dashed
   theme_bw() +
  theme_classic(base_size = 15) +
  geom_smooth(aes(group = randomization_group_factor), method = "lm", se = FALSE) +  # Adding best fit line by group
...
long_df_hvlt <- techs %>%
  select(name, randomization_group_factor, hvlt_IR_all, hvlt_IR_all_end) %>%
...
ggplot(long_df_hvlt, aes(x = timepoint, y = value, group = name, color = as.factor(randomization_group_factor))) +
  geom_line(linetype = "dashed") +  # Set individual lines as dashed
   theme_bw() +
  theme_classic(base_size = 15) +
...
long_df_hvlt_delay <- techs %>%
  select(name, randomization_group_factor, t4_correct, t4_correct_v2) %>%
...
ggplot(long_df_hvlt_delay, aes(x = timepoint, y = value, group = name, color = as.factor(randomization_group_factor))) +
  geom_line(linetype = "dashed") +  # Set individual lines as dashed
  theme_bw() +
  theme_classic(base_size = 15) +
  geom_smooth(aes(group = randomization_group_factor), method = "lm", se = FALSE, size = 1.5) +  # Adding best fit line by group
...
long_df_hvlt_recognition <- techs %>%
  select(name, randomization_group_factor, recognition_score, recognition_score_v2) %>%
...
ggplot(long_df_hvlt_recognition, aes(x = timepoint, y = value, group = name, color = as.factor(randomization_group_factor))) +
  geom_line(linetype = "dashed") +  # Set individual lines as dashed
  theme_bw() +
  theme_classic(base_size = 15) +
   geom_smooth(aes(group = randomization_group_factor), method = "lm", se = FALSE, size = 1.5) +  # Adding best fit line by group
...
ggplot(techs, aes(x = factor(age), fill = as.factor(randomization_group))) +
  ...
  scale_fill_manual(values = c("black", "red"))
ggplot(techs, aes(x = as.factor(randomization_group_factor), y = age, fill = as.factor(randomization_group_factor))) +
  geom_violin() + 
  ...
  scale_fill_manual(values = c("black", "red"))
#histogram of age by group
ggplot(techs, aes(x = factor(days_since_injury), fill = as.factor(randomization_group_factor))) +
  geom_histogram(stat = "count", position = "dodge") +  # Adjusting position of bars
  labs(x = "Category", y = "Count", title = "Days Since Injury") +
  scale_fill_manual(values = c("black", "red"))
ggplot(techs, aes(x = as.factor(randomization_group_factor), y = days_since_injury, fill = as.factor(randomization_group_factor))) +
  geom_violin() + 
  geom_point(position = position_jitterdodge(), size = 2, color = "white") +
  stat_summary(fun = mean, geom = "text", aes(label = round(..y.., digits = 2)),
               vjust = 1.5, size = 4, color = "white") +
